...
public class AnnounceSettingsEditor extends EditorMenu<DungeonPlugin, AnnounceSettings> implements AutoPaged<DungeonStage> {
    ...
    public AnnounceSettingsEditor(@NotNull AnnounceSettings settings) {
        ...
        this.addReturn(31).setClick((viewer, event) -> NexPlugin.getScheduler().runTask(SchedulerType.SYNC, viewer.getPlayer(), task -> settings.dungeon().getEditor().open(viewer.getPlayer(), 1), null));
        this.addNextPage(32);
